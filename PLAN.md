...
T14 修订版真实 OMP 咨询验收                  (T13)           → blocked [ACCEPTANCE_ADVISORY.md AC-L01/L03/L04/L05/C01/C02/C03 — AC-L02 RETIRED per D-08] AC-L04 PASS via managed-terminal BoilingCheetah 39.2s public TUI off→disabled→completed, AC-L05 SOURCE GAP, AC-C03 NOT OBSERVED 前保持 blocked — SPEC §22.2 NOT COMPLETE
T15 可配置按模型自动 effort 阶梯            (T13)           → done [src/core/config.ts, src/core/types.ts, src/core/decision.ts, src/core/state.ts, src/core/audit.ts, src/core/status.ts, src/extension.ts, PRD.md, SPEC.md, TECHNICAL_DESIGN.md, README.md, README.txt, test/mechanical.test.ts, test/extension.integration.test.ts, test/effortPolicy.test.ts] commit 398681b — 13 ACs AC-E01..E13 (AC-01..13) + post-set actual readback + settings lifecycle (complete loads, on/off preserves policy, invalid degrades without mutation) + no hardcoded real model names in src (scan 0 matches) + native task advisory unchanged — verified: node --experimental-strip-types --loader ./scripts/node-ts-loader.mjs test/effortPolicy.test.ts 49/49, same loader test/extension.integration.test.ts 126/126, npm test 66/66, npm run typecheck:core exit 0 — authority: docs/spark/2026-08-21-configurable-model-effort-policy-design.md @ bfef9fd + Owner implementation authorization 2026-08-21
```
...
> 2026-08-20 咨询实现收敛（Owner 批准，commit 48621b9）+ D-08 修正（AC-L02 RETIRED → AC-A18）：T10 规划迁移已于 8ea7f34 完成；T11/T12/T13 已于 commit 48621b9 实现并通过机械/类型校验（npm run typecheck:core exit 0, npm run check:types 32 members verified, `node --experimental-strip-types --loader ./scripts/node-ts-loader.mjs test/extension.integration.test.ts` → 110 passed 0 failed incl. AC-A18 deterministic official-hook handoff, POLICY 35 tokens ≤60），状态 done；T14 修订版真实 OMP 咨询验收因 ACCEPTANCE_ADVISORY.md 中 AC-L04 PASS, AC-L05 SOURCE GAP, AC-C03 NOT OBSERVED 保持 blocked（AC-L02 RETIRED per D-08, 已排除；AC-L04 now PASS via BoilingCheetah 39.2s public TUI, AC-L05/C03 remain），未升格为 done；原先无 in_progress 已结束。
> 2026-08-21 可配置按模型自动 effort 阶梯授权（Owner-approved design commit bfef9fd + Owner implementation authorization 2026-08-21）：新增 T15 依赖已满足的 T13（不依赖 blocked T14）；T15 已于 commit 398681b 完成 — 13 ACs AC-E01..E13（AC-01..13）+ post-set actual readback + settings lifecycle（complete loads, on/off preserves policy, invalid degrades without mutation）+ src 无硬编码真实模型名（forbidden scan 0 matches）+ native task advisory unchanged 均已验证（node --experimental-strip-types --loader ./scripts/node-ts-loader.mjs test/effortPolicy.test.ts 49/49, same loader test/extension.integration.test.ts 126/126, npm test 66/66, npm run typecheck:core exit 0）；T14 保持 blocked、T09 保持 pending、历史 T01..T13 原样保留；无 in_progress 剩余。
...
- **T14**：修订版真实 OMP 咨询验收保持 blocked，直至 ACCEPTANCE_ADVISORY.md 对 active 门 AC-L01/L03/L04/L05/C01/C02/C03 逐项具备真实 OMP PASS 证据（AC-L02 RETIRED per D-08, 排除）；当前 AC-L04 PASS (BoilingCheetah 39.2s public TUI off while waiting 2.1s→disabled detached, same child completed size 262B len260), AC-L05 SOURCE GAP (no in-runtime session_switch via public TUI), AC-C03 NOT OBSERVED (no <=200 audit copy via public TUI), 故保持 blocked（AC-L01 PASS, AC-L03 PASS with subfield SOURCE GAP, AC-C01 PASS, AC-C02 PASS, AC-A18 PASS；SPEC §22.2 NOT COMPLETE）。私有的 session JSONL/config 读取已执行但为不可采信观察，已排除。不得将机械测试或私有读取升格为 revised live done。
- **T15**：已完成（commit 398681b）：可配置按模型自动 effort 阶梯 — 权威 `docs/spark/2026-08-21-configurable-model-effort-policy-design.md` @ bfef9fd + Owner implementation authorization 2026-08-21，依赖仅 T13（不依赖 blocked T14，已满足）；精确验收 13 项 AC-E01..E13（AC-01..13）+ post-set actual readback 读回一致 + settings lifecycle（complete loads, on/off preserves policy, invalid degrades without mutation）+ 源码 `src` 内无硬编码真实模型名（scan 0 matches）+ native `task` 保持 advisory 不阻断均已通过；证据：commit 398681b, node --experimental-strip-types --loader ./scripts/node-ts-loader.mjs test/effortPolicy.test.ts → 49/49 PASS, same loader test/extension.integration.test.ts → 126/126 PASS, npm test → 66/66 PASS, npm run typecheck:core → exit 0, forbidden real-model-name scan → no matches；无 in_progress 剩余，T14 保持 blocked。
