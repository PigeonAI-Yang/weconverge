...
T03 计划 + 机器台账             (T02)           → in_progress [PLAN.md, ledger.json]
T04 实现纯核心引擎             (T03)           → pending [src/core/*]        AC-001..044(引擎)
...
T07 安装 junction + 真实OMP报告 (T06)          → pending [extensions/weconverge -> repo; ACCEPTANCE.md] AC-101..115
...
- 本沙箱无可驱动 OMP 运行时 → AC-101..115 真实读回维环境 BLOCKED，不伪造；机械维 AC-001..044 全过即视为实现完成的可验证部分。
