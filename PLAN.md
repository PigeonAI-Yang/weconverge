...
T15 可配置按模型自动 effort 阶梯            (T13)           → in_progress [src/core/config.ts, src/core/types.ts, src/core/decision.ts, src/core/state.ts, src/core/audit.ts, src/core/status.ts, src/extension.ts, PRD.md, SPEC.md, TECHNICAL_DESIGN.md, README.md, README.txt, test/mechanical.test.ts, test/extension.integration.test.ts] 13 design ACs AC-01..AC-13 + post-set actual readback + no hardcoded real model names in src + native task advisory unchanged — authority: docs/spark/2026-08-21-configurable-model-effort-policy-design.md @ bfef9fd + Owner implementation authorization 2026-08-21 — acceptance pending until implementation target suite passes
```
...
> 2026-08-21 可配置按模型自动 effort 阶梯授权（Owner-approved design commit bfef9fd + Owner implementation authorization 2026-08-21）：新增 T15 依赖已满足的 T13（不依赖 blocked T14），状态 in_progress 为当前唯一进行中任务；T14 保持 blocked、T09 保持 pending、历史 T01..T13 定性原样保留；T15 验收保持 pending 直至实现目标套件通过（13 design ACs、post-set actual readback、src 无硬编码真实模型名、native task 保持 advisory 不变），不得伪造 verifiedBy。
...
- **T15**：可配置按模型自动 effort 阶梯保持 in_progress — 权威：`docs/spark/2026-08-21-configurable-model-effort-policy-design.md` @ bfef9fd + Owner implementation authorization 2026-08-21，依赖仅 T13（不依赖 blocked T14，已满足）；契约范围 `src/core/config.ts, src/core/types.ts, src/core/decision.ts, src/core/state.ts, src/core/audit.ts, src/core/status.ts, src/extension.ts, PRD.md, SPEC.md, TECHNICAL_DESIGN.md, README.md, README.txt, test/mechanical.test.ts, test/extension.integration.test.ts` 仅作路径清单、未声称已存在或已通过；精确验收为 13 项设计 ACs（AC-01..AC-13）+ post-set actual readback 读回一致 + 源码 `src` 内无硬编码真实模型名 + native `task` 保持 advisory 观察不阻断；验收保持 pending 直至实现目标套件通过，不得伪造 verifiedBy；当前唯一 in_progress。
