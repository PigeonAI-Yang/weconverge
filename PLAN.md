...
T13 纯咨询机械验收                           (T11,T12)       → done [test/mechanical.test.ts] POLICY 35 ≤60, 65 passed 0 failed
T14 修订版真实 OMP 咨询验收                  (T13)           → blocked [ACCEPTANCE_ADVISORY.md AC-L01..L05/AC-C01..C03] 需真实 OMP 回读，SOURCE GAP/NOT OBSERVED 前保持 blocked
```
...
> 2026-08-20 咨询实现收敛（Owner 批准，commit 48621b9）：T10 规划迁移已于 8ea7f34 完成；T11/T12/T13 已于 commit 48621b9 实现并通过机械/类型校验（npm run typecheck:core exit 0, npm run check:types 32 members verified, mechanical POLICY 35 tokens ≤60, 65 passed 0 failed），状态 done；T14 修订版真实 OMP 咨询验收因 ACCEPTANCE_ADVISORY.md 中 AC-L02 SOURCE GAP、AC-L04 NOT OBSERVED (PARTIAL)、AC-L05 SOURCE GAP、AC-C03 NOT OBSERVED 保持 blocked，未升格为 done；当前无 in_progress，满足至多一个进行中约束（仅 T14 blocked 剩余，零 in_progress 合法）。
...
- **T13**：已完成（commit 48621b9）：咨询契约机械验收分层通过 — 策略注入有界性 (POLICY 35 ≤60)、fact taxonomy (requested/expected/observed/inferred/source_gap) 隔离、observer fail-open、audit 脱敏与截断（≤200 chars）、单活运行时 + bounded tombstone 语义。证据：node --experimental-strip-types --loader ./scripts/node-ts-loader.mjs test/mechanical.test.ts → POLICY tokens=35 budget=60; 65 passed, 0 failed; npm run typecheck:core exit 0; node scripts/check-ledger.mjs exit 0。
- **T14**：修订版真实 OMP 咨询验收保持 blocked，直至 ACCEPTANCE_ADVISORY.md 对 AC-L01..L05/AC-C01..C03 逐项具备真实 OMP PASS 证据；当前 AC-L02 SOURCE GAP、AC-L04 NOT OBSERVED (PARTIAL)、AC-L05 SOURCE GAP、AC-C03 NOT OBSERVED，故保持 blocked（AC-L01 PASS, AC-L03 PASS with subfield SOURCE GAP, AC-C01 PASS, AC-C02 PASS；SPEC §22.2 NOT COMPLETE）。不得将机械测试或 E-010 smoke 升格为 revised live done。
...
- 纯咨询退役项（§13 D-01..D-07）：enforcement（block/mutation/cancel/automatic dispatch/max preflight/wave/concurrency/duplicate/compact 输出强制）已显式退役为 advisory 观察，不在新链中作为完成门；旧 AC-101..115 的 NOT COMPLETE/REJECTED 保持历史真实，不重写为 PASS。
- 修订版门：AC-L01..L05/AC-C01..C03 由 ACCEPTANCE_ADVISORY.md 单独判定；T14 的 blocked 以该文件为准，不复用 ACCEPTANCE.md 的 AC-101..115 判定；历史 T07 仍以 ACCEPTANCE.md 的 AC-101..115 为准。
