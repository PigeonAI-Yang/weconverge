...
**SPEC §22.2 (v1.1-advisory): NOT COMPLETE.** Mechanical layer AC-A01..A17 and cost AC-C01..C02 are deterministically PASS. Live AC-L01 and AC-L03 are PASS (with documented sub-field SOURCE GAP). AC-L02 and AC-L05 remain **SOURCE GAP**, AC-L04 remains **NOT OBSERVED (PARTIAL)**, AC-C03 remains **NOT OBSERVED**. No retired AC is promoted to PASS. Owner final acceptance remains open. Historical `ACCEPTANCE.md` (:1–329, REJECTED/NOT COMPLETE) and `REVIEW.md` (:1–56, T08 PASS deterministic-only) are preserved.

| Layer | Pass condition | Result |
|---|---|---|
| Mechanical AC-A01..A17 | All 17 PASS deterministic | **17/17 PASS** — `POLICY tokens=35 budget=60; 65 passed, 0 failed` (see §2) |
| Live AC-L01..L05 | All 5 observed PASS without SOURCE GAP/NOT OBSERVED promotion | **2 PASS, 2 SOURCE GAP, 1 NOT OBSERVED (PARTIAL)** — AC-L01 PASS, AC-L03 PASS (with subfield SOURCE GAP), AC-L02 SOURCE GAP, AC-L05 SOURCE GAP, AC-L04 NOT OBSERVED (PARTIAL) |
| Cost AC-C01..C03 | All 3 PASS observed | **2 PASS, 1 NOT OBSERVED** — AC-C01 PASS, AC-C02 PASS, AC-C03 NOT OBSERVED |
| Retired enforcement | No RETIRED AC marked PASS | **HONORED** — AC-010/011/012/013/016-018/019/036/040/105/107/108 recorded RETIRED |
| Overall SPEC §22.2 | All live required gates observed | **NOT COMPLETE** — blocked by AC-L02/L04/L05 and AC-C03 |
...
- Command: `node --experimental-strip-types --loader ./scripts/node-ts-loader.mjs test/mechanical.test.ts`
- Supplied result line (authoritative, not re-executed by this report): `POLICY tokens=35 budget=60; 65 passed, 0 failed`
...
- Paths: `src/core/{policy,observation,types,state,decision,audit,ledger,config,route,cost,status,commands,evidence,ids}.ts`, `src/extension.ts`, `test/mechanical.test.ts`, `types/omp-extension-api.d.ts`
- Boundary: Fake `ExtensionAPI` only. Proves policy boundedness, taxonomy separation, observer fail-open, decide narrow gate, parent-effort ladder, tombstone bounds, audit sanitization/truncation, command idempotency. **Cannot prove live OMP observation** (no junction, no TUI, no Provider payload, no `resolvedModel` readback, no `session_switch`).
...
## 3. Mechanical acceptance — AC-A01..A17 (deterministic, fake ExtensionAPI)

Result basis: supplied mechanical `65 passed, 0 failed` with `POLICY tokens=35 ≤ 60` under advisory contract. Each AC below has exact evidence path; no live claim is derived from mechanical.
...
All 17 mechanical gates are PASS on deterministic evidence. None implies a live PASS.

---

## 4. Live OMP observation — AC-L01..L05 (observed, not enforced)

Mechanical results do not upgrade live gates. Constraints per assignment: AC-L02 and AC-L05 remain SOURCE GAP, AC-L04 remains NOT OBSERVED (PARTIAL), no overclaim.
...
| **AC-L02** strategy injection: first new generation `before_agent_start` observed `systemPrompt` containing ≤60-token policy | **SOURCE GAP** | **Observed:** generation 1 enabled `enabled:true, phase:baseline` (E-L03.3 `status` JSON). **Expected:** `POLICY_BLOCK` 35 (current 33) ≤60 deterministic. **Source gap:** per-request `systemPrompt` content in Provider payload not exposed via any public Extension API (`before_provider_request.payload: unknown`, no TUI/Provider payload dump). E-L04 notes policy effect consistent with subsequent `task` batch but direct systemPrompt readback honest **SOURCE GAP** per design `CAPABILITY_PROBE` + `docs/spark/2026-08-20-weconverge-pure-advisory-design.md §4.1`. No synthesis; remains SOURCE GAP. |
...
Per PRD §0.2/§0.3 and SPEC §0.2/§0.3 / §18–19 consultation disposition, the following historical enforced gates are **RETIRED (advisory)** under v1.1-advisory and are recorded here as RETIRED, never as PASS:
...
This path proves: pure-advisory load, command lifecycle, generation-scoped ≤60-token policy (deterministic 35 ≤ 60), one native task batch of 2 observed via `tool_call`/`tool_result`/`task:subagent:lifecycle`, and advisory `off` — without a second scheduler, wrapper, blocking, mutation, or cancellation.

---

## 8. Exact remaining gates (why NOT COMPLETE)

Completion stays **NOT COMPLETE** until live required gates have observed evidence (no synthetic promotion):

| Gate | What would close it |
|---|---|
| **AC-L02** per-request `systemPrompt` payload | A future smoke that dumps the Provider request actually sent at generation start and shows `POLICY_BLOCK` present with ≤60 tokens per request — requires a public Provider-payload readback surface not currently available (expected to stay SOURCE GAP until OMP exposes it). Generation existence alone is not sufficient. |
...
# 2) Deterministic mechanical — no OMP runtime
node --experimental-strip-types --loader ./scripts/node-ts-loader.mjs test/mechanical.test.ts
# → POLICY tokens=35 budget=60; 65 passed, 0 failed (E-L04 budget, §2)
